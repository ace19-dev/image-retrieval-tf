...
def main(_):
    ...
    for i, cls in enumerate(cls_lst):
        ...
        for idx, img in enumerate(img_lst):
            ...
            img_cls = img.split('.')[0].split('_')[1]
            img_cls_path = os.path.join(FLAGS.target_dir, img_cls)
            if not os.path.exists(img_cls_path):
                os.makedirs(img_cls_path)

            old_img_path = os.path.join(cls_path, img)
            new_img_path = os.path.join(img_cls_path, img)
